...
created: '2026-09-21'
status: 'done'
baseline_commit: '441d2633412916bd9fba3e367b1e29a4cdc5f902'
route: 'dispatch'
...
- `packages/web/e2e/fixtures.ts` -- Story 1.7's `test` with the `api` fixture: `openAccount` (EUR only today; add a `currency` option for the USD case), `addTransaction`, `addDailyTransactions` (sequential, one per day), `recordSnapshot`, `groupTotal`, plus `uniqueName`, `daysAgo`, `typed`, `euros`. One database serves the whole run with `workers: 1`, so `/operations` lists every test's transactions: each test narrows the list to its own rows with `account=` or a `uniqueName` in `q` before asserting order, count, total or pagination. `e2e/transactions.spec.ts` shows the sheet and row locators to reuse.
- `packages/web/src/routeTree.gen.ts` -- regenerated by the router plugin when `operations.tsx` is added; commit it, since CI fails on any tracked file changed by `vite build`.
- `~/github/sure/app/models/entry_search.rb` (L16–58) -- text, date and absolute amount filters.

## Tasks & Acceptance

**Execution:**
- [x] `packages/data/schema/transactions.ts`, `entries.ts`, `drizzle/0003_*.sql` -- `excluded` column, lockable, and `entries_kind_date` index.
- [x] `packages/api/src/domain/transaction-filter.spec.ts`, `transaction-filter.ts` (new) -- tests first: `amountBoundsFor(min, max, currency)` (inward rounding, absolute) and `escapeLike(q)`. 100% branches.
- [x] `packages/api/src/services/ledger.spec.ts`, `ledger.ts` -- tests first: filtered list, count and sum per currency, `excluded` patch and lock, a 50,000-row seeded first page timed under 300 ms; then the code.
- [x] `packages/api/src/schemas/transactions.ts`, `services/transactions.ts`, `routes/transactions.ts`, `app.spec.ts` -- `GET /api/transactions` and `excluded` on `PATCH`; the spec covers the I/O matrix.
- [x] `packages/web/src/components/ui/switch.tsx`, `badge.tsx` -- `pnpm dlx shadcn add switch badge`, patched for oxlint as other `ui/` files.
- [x] `packages/web/src/lib/transaction-filters.spec.ts`, `transaction-filters.ts` (new) -- search schema (`.catch(undefined)` per param) and chip labels, tested.
- [x] `packages/web/src/lib/query-keys.ts`, `hooks/useTransactions.ts` -- `transactions.list(filters, page)` under `transactions.all`, `useTransactions(filters, page)`, invalidation.
- [x] `packages/web/src/routes/operations.tsx` (new), `components/TransactionFilters.tsx` (new), `TransactionList.tsx`, `Money.tsx` or an `ExcludedAmount` wrapper, `TransactionSheet.tsx`, `Pagination.tsx`, `AppSidebar.tsx`, `locales/fr.json` -- page, filters, chips, count and total, marker, switch, nav item.
- [x] `packages/web/e2e/operations.spec.ts` (new) -- Playwright: cross-account order and account name, each filter alone and combined, reload keeping chips, removing a chip, « Effacer les filtres », count and total with a USD account skipped, pagination at 51 rows, exclusion marker on both lists with the header balance unchanged.
...
- The text filter is the only `sql` fragment in `ledger.ts`: Drizzle's `like` has no `escape` clause, and without one `50%` matches `Remise 500`. The escape character is bound as a parameter. The absolute amount bounds need none: they are two `between` ranges, one per sign.
- Amount bounds are parsed once by `parseAmountBound` in `schemas/transactions.ts` into an exact `{ units, scale }` decimal, then scaled per account currency by `amountBoundsFor`. A signed bound is `invalid_amount`: the sign is Epic 5's direction filter.
- The exclusion switch shows in the sheet when editing only; `POST` does not accept `excluded`, since `NormalizedTransaction` (AD-3) has no such field.
- Items of `GET /api/accounts/:id/transactions` carry `accountName` too, so both lists share one row type.
- 50,000 seeded rows: list, count and sum answer in about 40 ms on a local file, so no FTS5.

- Verified in Chromium on a throwaway database: a checking and a card account, six transactions, `MAIF assurance` excluded. `/operations` reads « 6 résultats · Total : +1 675,13 € », the excluded row muted with the eye-off icon; `account=<checking>` with `q=carrefour` leaves « Carrefour Market » alone. Light 1280 px and dark 600 px checked. The account-name column is not aligned across rows, since the amount's width varies.
...
|---|--------|----------|---------|---------|----------|-------|
| 1 | blind, edge, verification | `lib/transaction-filters.ts` `operationsSearchSchema`, `operations.tsx` error block | A link with `to` before `from`, `amountMin` above `amountMax` or an over-long `q` leaves the page on an error whose retry always fails | medium | Each param validates alone; the API answers 400; the schema's comment promises such values are dropped. | patch |
| 2 | blind | `lib/transaction-filters.ts` | A single `?account=<id>` is silently dropped | medium | The router hands a string to `z.array(...).min(1)`, `.catch` drops it; the API accepts that form. | patch |
| 3 | verification | `e2e/operations.spec.ts` | No test visits a page past the last one | medium | Removing `useClampPage` from `operations.tsx` passes every test. | patch |
| 4 | verification | `e2e/operations.spec.ts` | The matrix's bad-amount row has no interface test | medium | Removing `below_min` from `AmountEditor.apply` passes every test. | patch |
| 5 | blind | `schemas/transactions.ts` `parseAmountBound`, `compareAmountBounds` | Money parsing tested through two route cases only | medium | Separators, grouping and scales are untested branches of a money path. | patch |
| 6 | blind, edge | `ledger.ts` `TransactionFilter.q` | "Case-insensitive" holds for ASCII only | low | SQLite `LIKE` folds A to Z; the frozen intent accepts it, the comment must say it. | patch |
| 7 | blind, verification | `ledger.ts` `updateTransaction` | Toggling `excluded` recomputes balances and bumps `updated_at` | low | Pre-existing for `label` and `notes`; the result is unchanged and tested. Rejected. |  |
| 8 | blind | `ledger.spec.ts` performance | The `q` path is not timed; 300 ms may flake on CI | low | The criterion is the unfiltered first page, measured at about 40 ms. Rejected. |  |
| 9 | blind | `useInvalidateAccount`, `queryKeys.transactions.all` | Account renames and deletes will not refresh `accountName` on `/operations` | low | No account write but creation exists before Story 1.6, and creation adds no row to the list. | defer |
| 10 | blind | `services/transactions.ts` `listAllTransactions` | Four reads outside one transaction; the count duplicates the sum's counts | low | Same as `listTransactions` since Story 1.2; a stale total self-corrects. Rejected. |  |
| 11 | blind, edge | `operations.tsx` `OperationSheet` | Nothing shows while the account loads, nor on its error | low | The account detail is one indexed read, usually cached. Rejected. |  |
| 12 | blind | `schemas/transactions.ts` | Bounds parsed twice; `AmountBound` duplicates `DecimalAmount` | low | No wrong result; rejected. |  |
| 13 | edge | `ledger.ts` `sumTransactions` | A sum past `Number.MAX_SAFE_INTEGER` throws | false | That is 90 trillion euros for one household. |  |
| 14 | edge | `TransactionFilters.tsx` | More than 100 accounts selected gives a 400 | low | A household has a handful of accounts; rejected. |  |
| 15 | edge | `TransactionFilters.tsx` | Unknown ids in the URL are resent on apply | low | Needs a deleted account, which arrives in Story 1.6; rejected. |  |
| 16 | edge | `lib/transaction-filters.ts` | `?q=true` or `?q=1e5` parsed as JSON by the router | low | Hand-typed only; rejected. |  |
| 17 | blind | spec, `sprint-status.yaml` | Statuses disagree | false | Step 5 sets both to their final state. |  |
| 18 | blind | review diff | Migration journal and route tree missing from the diff | false | Left out of the review file on purpose; present in the tree and applied by the e2e run. |  |
| 19 | verification | `entries_kind_date` | No test fails if the index disappears | low | The target holds without it (11 ms); a query-plan assertion belongs with the volume that needs it. | defer |
